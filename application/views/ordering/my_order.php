...
<li class="list-group-item d-flex justify-content-between lh-condensed col-md-12 order-list">
        <table width="100%">
            <tr>
                <td width="80%">
                <div>
                        <h6 class="my-0 ref-field">Producto:</h6>
                        <small class="text-muted">
                           <p class="mt-2">Productos: <span class="items-count">3</span></p>
                           <p class="mt-1">Monto Total:<span class="total-amount">1500</span></p>
                        </small>
                    </div>
                </td>
                <td width="20%">
        ...
        <li class="list-group-item d-flex justify-content-between lh-condensed col-md-12 od-list">
            ...
            <tr>
                ...
                <td width="100%" colspan="2">
                    <div class="text-muted"><h4 class="pname">Producto:</h4></div>
                </td>
            </tr>
            <tr>
      ...
      <div class="modal-dialog modal-full-height modal-left" role="document">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title w-100" id="od_modal">Detalle de pedido</h4>
                ...
                <table width="100%">
                    <tr>
                        <td width="50%" style="vertical-align:top">
                            <div class="text-muted"><small>Cod. Pedido :</small></div>
                             <h5><b class="ref_no">asdad asdasd</b></h5>

                            <div class="text-muted"><small>Estado :</small></div>
                            <div class="status"></div>

                            <div class="text-muted"><small> Otros :</small></div>
                            <div><b class="remarks"></b></div>

                        </td>
                        <td width="50%" style="vertical-align:top">
                        <div class="text-muted"><small>Tipo de Pedido:</small></div>
                        <div><b class="otype"></b></div>
                        <div class="del_only">
                        <div class="text-muted"><small> Dirección :</small></div>
                        <div><b class="loc"></b></div>
                        <div class="text-muted"><small> Referencia :</small></div>
                ...
                <div class="col-md-12">
                    <ul class="list-group mb-2 mt-2 z-depth-1" id="od-field">
                        ...
                        <li class="list-group-item d-flex justify-content-between lh-condensed col-md-12 total">
                                <table width="100%">
                                    <tr>
                                        <td width="30%"><h5><b>Total a pagar</b></h5></td>
            ...
            $.ajax({
                ...
                success:resp=>{
                    if(typeof resp != undefined){
                        ...
                        $('#order_details .status').html('<span class="badge badge-success">Entregado<span>')
                        if(resp.details.status == 1 && resp.details.type == 4)
                        $('#order_details .status').html('<span class="badge badge-success">Recogido<span>')
                        if(resp.details.status == 0)
                        $('#order_details .status').html('<span class="badge badge-primary">Pendiente<span>')
                        resp.details.total_amount = resp.details.total_amount.replace(/,/g,'')
                        if(resp.details.total_amount > 0)

                        if(resp.details.type == 3){
                        $('#order_details .otype').html('Por Delivery')
                        $('#order_details .del_only').show()
                        }
                        if(resp.details.type == 4){
                        $('#order_details .otype').html('Para Recoger')
                        ...
                        resp.details.remarks = resp.details.remarks != '' ? resp.details.Otros : 'N/A'
                ...
                if(Object.keys(resp).length >0 ){
                    Object.keys(resp).map((k)=>{
                        var li = $('#order_list_clone .order-list').clone()
                            ...
                            if(resp[k].status == 1){
                                if(resp[k].type == 3)
                                li.find('.status').addClass('badge-success').html('<i>Entregado</i>')
                                if(resp[k].type == 4)
                                li.find('.status').addClass('badge-success').html('<i>Recogido</i>')
                            }else{
                            li.find('.status').addClass('badge-primary').html('<i>Pendiente</i>')
                            }
